    ...
    async execute(message) {
        ...
        try {
            ...
            if (guildS && guildS.messageCounts) {
                const { UserData } = require("../models.sql");
                const msgCount = await UserData.findOne({ where: { guildId: message.guildId, userId: message.author.id } });
                if (!msgCount) {
                    const newMsgCount = await UserData.create({id: generateRandomNumber(), userId: message.author.id, guildId: message.guildId, count: 1});
